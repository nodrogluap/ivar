...
\DoxyCodeLine{           -\/i    (Required) Sorted bam file, with aligned reads, to trim primers and quality}
...
Example Usage\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{bwa mem -\/t 32 reference.fa 1.fq 2.fq | samtools view -\/h -\/F 4 -\/F 2048 | samtools sort -\/ | ivar trim -\/b test\_primers.bed | samtools sort -\/ | samtools mpileup -\/aa -\/A -\/Q 0 -\/d 0 -\/ | ivar consensus -\/p test\_consensus -\/m 10 -\/n N -\/t 0.5}
